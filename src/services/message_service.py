...
class MessageService(Base):
    def __init__(self,db:AsyncSession,customer_id:UUID,conversation_id:UUID,agents:dict):
        super().__init__()
        self.db=db
    ...
    async def send_message(self, content:SendMessages):
        ...
        try:
            ...
            query_service = QueryService(db=self.db,company_db_service=CompanyDBServices(), company_database_url=company_db_url)
            order_service=OrderActionService(
            db=self.db,
            company_db_service=CompanyDBServices(),
            company_database_url=company_db_url,
             )
            context = AgentContext(query_service=query_service, order_action_service=order_service)
            ...
            response,message_id = await agent_service_manger.run_agent(
                messages,customer_id=self.customer_id, conversation_id=self.conversation_id, context=context, company_id=conversation_info.company_id,agent_id=agent_info.id
            )
